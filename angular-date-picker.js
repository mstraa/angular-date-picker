(function(root, factory) {
    ...
    return angular
        .module("mp.datePicker", [])
        .run(function($templateCache) {
            $templateCache.put(
                "datepicker-template-default.html",
                "" +
            ...
            function($window, $locale) {
                ...
                return {
                    ...
                    scope: {
                        ...
                        events: "=" //object with date as key date format "YYYY-MM-DD"
                    },
                    ...
                    link: function($scope, $element, $attributes, ngModel) {
                        var selectedDate = null,
                            ...
                            daysOfWeek = [],
                            firstDayOfWeek =
                                typeof $locale.DATETIME_FORMATS
                        ...
                        for (var i = 0; i < 12; i++) {
                            months.push({
                                ...
                                shortName:
                                    $locale.DATETIME_FORMATS.SHORTMONTH[i]
                            });
                        }
                        ...
                        function setYearAndMonth(date) {
                            ...
                            $scope.today =
                                now.getFullYear() === $scope.year &&
                                now.getMonth() === $scope.month
                                    ? now.getDate()
                                    : null;
                            ...
                            var firstDayOfMonth = new Date(
                                ...
                                leadingDays =
                                    (dayOfWeek - firstDayOfWeek + 7) % 7 || 7; // Ensure there are always leading days to give context

                            $scope.leadingDays = days.slice(
                                -leadingDays - (31 - daysInLastMonth),
                                daysInLastMonth
                            );
                            $scope.days = days.slice(0, daysInMonth);
                            // Ensure a total of 6 rows to maintain height consistency
                            $scope.trailingDays = days.slice(
                                0,
                                6 * 7 - (leadingDays + daysInMonth)
                            );
                        }
                        ...
                        $scope.pickDay = function(evt) {
                            ...
                            if (target.hasClass("_day")) {
                                var monthOffset = target.attr(
                                    "data-month-offset"
                                );

                                if (monthOffset) {
                                    $scope.changeMonthBy(
                                        parseInt(monthOffset, 10)
                                    );
                                }
                        ...
                        $scope.eventsThisDay = function(date) {
                            return (
                                $scope.events[date] &&
                                $scope.events[date].length > 0
                            );
                        };
                    }
                };
